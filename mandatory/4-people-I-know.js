...
const thirtyFiveOrOlder = people.filter(friend => friend.age >= 35);
/*
...
const friendsWithStacie = [];
people.forEach(friend => {
	const friends = friend.friends.map(person => person.name);
	if (friends.includes('Stacie Villarreal')) {
		friendsWithStacie.push(`${friend.name.first} ${friend.name.last}`);
	}
});
...
const friendsWhoCanMultitask = [];
const multiTaskingFriendsNames = people
	.flatMap(entry => entry.friends)
	.filter(entry => entry.skills.includes('Multi-tasking'))
	.map(entry => entry.name);
const rearranged = multiTaskingFriendsNames
	.splice(1, 4)
	.concat(multiTaskingFriendsNames)
	.reverse();
friendsWhoCanMultitask.push(rearranged);
